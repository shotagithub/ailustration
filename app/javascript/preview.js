document.addEventListener('DOMContentLoaded', function(){
  const imgForm = document.getElementById('new_post');
  const previewList = document.getElementById('previews');
  if (!imgForm) return null;
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'illust[images][]');

    const lastFileField = document.querySelector('input[type="file"][name="illust[images][]"]:last-child');
  ...
  const fileField = document.querySelector('input[type="file"][name="illust[images][]"]');
